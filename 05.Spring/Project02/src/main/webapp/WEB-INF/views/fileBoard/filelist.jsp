	...
	<table class="table">
	  ...
	  <tbody>
		...
		<tr>
			...
			<td>${dto.fName }</td>
			<td>
				<a href="filecontent_view?fId=${dto.fId }&kind=view">${dto.fTitle }</a></td>
			<td>${dto.fDate }</td>
			<td>${dto.fHit }</td>
		</tr>
		</c:forEach>
		...
		<tr>
			<td colspan="3"><a class="btn btn-outline-dark" href="upload_view">파일업로드</a></td>
			<td colspan="2"><a class="btn btn-outline-dark" href="main">메인</a></td>
		</tr>
		...
		<tr>
			...
			<c:otherwise>
				<a href="filelist?page=${fpage.curPage - 1}">[ &lt; ]</a>
			</c:otherwise>
			</c:choose>
			...
			<c:forEach var="fEach" begin="${fpage.startPage}" end="${fpage.endPage}" step="1">
				...
				<c:otherwise>
					<a href="filelist?page=${fEach}">[ ${fEach} ]</a>&nbsp;
				</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:otherwise>
				<a href="filelist?page=${fpage.totalPage}">[&gt;&gt;]</a>
			</c:otherwise>
			</c:choose>
			</td>
		</tr>
